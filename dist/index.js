...
function generateEDAPrompts(data, analysisType) {
    ...
    try {
        switch (name) {
            ...
            case "analyze-csv": {
                ...
                const summaryContent = responses.map(r => `=== Analysis Part ${r.part} ===\n\n${r.content}\n\n`).join('\n');
...
async function main() {
    try {
        ...
        const httpServer = app.listen(port, () => {
            const actualPort = port === 0 ? httpServer.address()?.port : port;
